...
DECLARE
  new_id int;
BEGIN
  INSERT INTO creative_cash_flow (user_id)
  VALUES (_user_id) RETURNING id
  INTO new_id;
...
CREATE TABLE creative_cash_flow_inputs (
  id int PRIMARY KEY NOT NULL REFERENCES public.creative_cash_flow(id) ON DELETE CASCADE,
  user_id uuid REFERENCES public.users(id) ON DELETE CASCADE NOT NULL,
  created_at timestamp with time zone NOT NULL DEFAULT timezone('utc'::text, now()),
...
RETURNS TABLE(id int, inputs jsonb, results jsonb) AS
$BODY$
BEGIN
    RETURN QUERY
        SELECT
