...
export const compare_strategies = (
  ...
  const diff_in_months = results.current.payoff_months - results.strategy.payoff_months;
  let currentSaved = 0;
  let currentCost = 0;
  let strategySaved = 0;
  let strategyCost = 0;
  ...
  if (results.current.total_amount < results.strategy.total_amount) {
    const val = currencyOperation(
      'subtract',
      results.strategy.total_amount,
      results.current.total_amount
    );
    currentSaved = strategyCost = val;
    currentDateDiff = -diff_in_months;
    strategyDateDiff = diff_in_months;
  } else if (results.current.total_amount > results.strategy.total_amount) {
    const val = currencyOperation(
      'subtract',
      results.current.total_amount,
      results.strategy.total_amount
  ...
  const months = inputs.pay_back_loan
    ? diff_in_months - results.strategy.loan_payback.months
    : diff_in_months;
